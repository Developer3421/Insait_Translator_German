...
public class TranslationService : IDisposable
{
    ...
    public async Task<TranslationResult> TranslateWithDetailsAsync(string text, string sourceLang = "uk", string targetLang = "de")
    {
        ...
        {
            var errorMessage = string.Format(LocalizationManager.Instance.Strings.TranslationError, ex.Message);
    ...
    private async Task<string> TranslateWithMyMemoryAsync(string text, string sourceLang, string targetLang)
    {
        ...
        {
            throw new MyMemoryQuotaExceededException(LocalizationManager.Instance.Strings.MyMemoryDailyQuotaExceeded);
        }

        if (responseStatus != 200)
        {
            var errorMessage = string.Format(LocalizationManager.Instance.Strings.MyMemoryApiError, responseStatus);
        ...
        {
            throw new MyMemoryQuotaExceededException(LocalizationManager.Instance.Strings.MyMemoryQuotaWarning);
        }
    ...
    private async Task<string> TranslateWithGoogleApiAsync(string text, string sourceLang, string targetLang)
    {
        ...
        if (string.IsNullOrEmpty(apiKey))
            throw new Exception(LocalizationManager.Instance.Strings.GoogleApiKeyNotConfigured);
    ...
    public async Task<(bool Success, string Message)> TestGoogleApiKeyAsync(string apiKey)
    {
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                return (true, LocalizationManager.Instance.Strings.ApiKeyValid);
            }
            else
            {
                var errorContent = await response.Content.ReadAsStringAsync();
                var errorMessage = string.Format(LocalizationManager.Instance.Strings.ApiKeyError, response.StatusCode, errorContent);
                return (false, errorMessage);
            }
        }
        catch (Exception ex)
        {
            var errorMessage = string.Format(LocalizationManager.Instance.Strings.ApiKeyConnectionError, ex.Message);
            return (false, errorMessage);
        }
    }
